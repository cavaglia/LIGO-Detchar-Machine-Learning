...
def filter_data(fs,fband,strain,DQ):
    if len(fband):
    ...
    for time in notnans_arrays:
        if len(fband):
            strain_conditioned_time = signal.filtfilt(bb, ab, strain[time]) / normalization
        else:
            strain_conditioned_time = strain[time]
...
#This function bandpasses the data with a fourth-order Butterworth 
#def filter_data(X,sampling_rate,lf,hf):
#    nyq_freq = sampling_rate / 2
#    lf_cut = lf / nyq_freq
#    hf_cut = hf / nyq_freq
#    b, a = signal.butter(4, [lf_cut,hf_cut], btype='bandpass')
#    Xfiltered = signal.filtfilt(b, a, X,method='pad')
#    return Xfiltered
...
strain_conditioned, DQ_conditioned = filter_data(sampling_rate,filter_freq,strain,DQ)
